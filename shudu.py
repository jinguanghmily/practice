...
# https://zhuanlan.zhihu.com/p/81584263
# 求每一行单元格行值域
def valueRange(row):
    ...
    for i in row:
        if i == '':
            ...
            if row_value_range.count(i) > 0:  # 返回元素在列表中出现的次数
...
def totalValueRange(soduku):
    ...
    for i in range(9):
        for j in range(9):
            total_value_range[i].append(
                inersection(row_value_range[i][j], col_value_range[i][j], matrix_value_range[i][j]))
